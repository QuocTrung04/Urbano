...
class HoaDonDetailView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final int status = item.trangThai ?? 1; // 1: Chưa thanh toán, 2: Đã thanh toán, 3: Thanh toán 1 phần
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (item.chiTietHoaDonList.isEmpty)
                        ...
                        ...item.chiTietHoaDonList.map((detail) => _buildDetailItem(context, detail)),
  ...
  Widget _buildHeaderCard(BuildContext context, HoaDonModel item, String statusText, Color statusColor) {
    return Container(
  ...
  Widget _buildDetailItem(BuildContext context, ChiTietHoaDon detail) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.borderButton.withValues(alpha: 0.5)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  Widget _buildPaymentSummary(BuildContext context, HoaDonModel item, bool isPaid) {
    final double tongTien = item.tongTien ?? 0;
    final double daThanhToan = item.soTienDaThanhToan ?? 0;
    final double conLai = tongTien - daThanhToan > 0 ? tongTien - daThanhToan : 0;
    ...
    return Container(
      ...
      child: Column(
        children: [
          _buildSummaryRow('Tổng hóa đơn', _formatPrice(tongTien), isBold: false),
          const SizedBox(height: 8),
          _buildSummaryRow('Đã thanh toán', _formatPrice(daThanhToan), isBold: false, valueColor: AppColors.tealPrimary),
          const Divider(color: AppColors.borderButton, height: 16),
          _buildSummaryRow('Còn lại cần thanh toán', _formatPrice(conLai), isBold: true, valueColor: isPaid ? AppColors.tealPrimary : AppColors.red),
        ],
      ),
    );
  }
  ...
  Widget _buildBottomAction(BuildContext context, HoaDonModel item, bool isPaid) {
    return Container(
